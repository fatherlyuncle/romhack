...
#include "event_data.h" //added for shop inventory to pull badgeCount
...
static const u16 *const sShopInventories[] = //Added this plus all above for shop inventory
{
...
static u8 GetNumberOfBadges(void) //Get badge number to set mart inventory
{
    u16 badgeFlag;
    u8 count = 0;
...
static void SetShopItemsForSale(const u16 *items)//Set shop inventory to depend on badgeCount
{
    u16 i = 0;
...
static void Task_ReturnToItemListAfterItemPurchase(u8 taskId)
{
    /*s16 *data = gTasks[taskId].data;   //old code 
